...
void FindNode_HasMesh(FbxNode* in_node, std::vector<FbxNode*>& node_array)
{
	...
	{
		FindNode_HasMesh(in_node->GetChild(i), node_array);
	}
	...
	MeshBase()
{
	...
	FindNode_HasMesh(fbx_root_node, node_has_mesh);
...
bool SkeltalMesh::CreateBuffer(VertexData* v, int v_num, unsigned int* indices, int i_num, ID3D11Buffer** v_buffer, ID3D11Buffer** i_buffer)
{
	HRESULT hr;
...
void SkeltalMesh::Render(float dt, DirectX::XMFLOAT3 pos, float angle, float scale)
{
...
void SkeltalMesh::Render(const Yukitter::Transform& transform)
{
...
void SkeltalMesh::Render(const Yukitter::Transform& transform, const Yukitter::Transform& parnt_transform)
{
	Vector camera_pos = GameBrain::GetInstance()->GetActiveCameraLocation();
	Vector camera_focus = GameBrain::GetInstance()->GetActiveCameraFocus();
	XMVECTOR eye = XMVectorSet(camera_pos.x, camera_pos.y, camera_pos.z, 1.0f);
...
void SkeltalMesh::Render(const Yukitter::Vector& scale, const DirectX::XMMATRIX& rotation_matrix, const Yukitter::Vector& translation)
{
	Vector camera_pos = GameBrain::GetInstance()->GetActiveCameraLocation();
	Vector camera_focus = GameBrain::GetInstance()->GetActiveCameraFocus();
	XMVECTOR eye = XMVectorSet(camera_pos.x, camera_pos.y, camera_pos.z, 1.0f);
...
void SkeltalMesh::Render(const DirectX::XMMATRIX& world_mat)
{
	XMFLOAT4X4 wvp;
	XMFLOAT4X4 inverse;
	Vector camera_pos = GameBrain::GetInstance()->GetActiveCameraLocation();
